...
public class PSPrinterJob extends RasterPrinterJob {
 ...
 /* Instance Methods */
   ...
   /**
	 ...
	 */
	if (index > 0) {
	    PageFormat prevPageFormat = getPageable().getPageFormat(index - 1); 
	    double prevPaperHeight = prevPageFormat.getPaper().getHeight();
	    double prevPaperWidth = prevPageFormat.getPaper().getWidth();

	    if ((paperHeight != prevPaperHeight) || 
		(paperWidth != prevPaperWidth)) {
