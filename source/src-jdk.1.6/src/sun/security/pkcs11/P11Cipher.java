        ...
        try {
            ...
            if (encrypt) {
                if (paddingObj != null) {
                    ...
                    k = token.p11.C_EncryptUpdate(session.id(), 0, padBuffer, 0,
                            actualPadLen, 0, out, outOfs, outLen);
                }
                k += token.p11.C_EncryptFinal(session.id(), 0, out,
                        (outOfs + k), (outLen - k));
            } else {
                if (paddingObj != null) {
                    if (padBufferLen != 0) {
            ...
            if (encrypt) {
                if (paddingObj != null) {
                    int actualPadLen = paddingObj.setPaddingBytes(padBuffer,
                            requiredOutLen - bytesBuffered);
                    k = token.p11.C_EncryptUpdate(session.id(), 0, padBuffer, 0,
                            actualPadLen, outAddr, outArray, outOfs, outLen);
                }
                ...
                if (paddingObj != null) {
                    ...
                    k = token.p11.C_DecryptFinal(session.id(), outAddr,
