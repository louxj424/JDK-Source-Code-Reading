...
class ForwardBuilder extends Builder {
	 ...
	 */
        if (currState.isInitial()) {
            certs.addAll(getMatchingEECerts(currState, certStores));
	}
        certs.addAll(getMatchingCACerts(currState, certStores));
	...
	(ForwardState currentState, List<CertStore> certStores) 
	throws CertStoreException, CertificateException, IOException {
	...
	if (currentState.isInitial()) {
            X509CertSelector targetCertConstraints = (X509CertSelector)
                buildParams.getTargetCertConstraints();
	    ...
	    if (caTargetSelector == null) {
	        caTargetSelector = targetCertConstraints;
	    ...
	    if (caSelector == null) {
		caSelector = new LDAPCertStore.LDAPCertSelector();
	...
	if (!searchAllCertStores && !forwardCerts.isEmpty()) {
	    return forwardCerts;
	}
	...
	{
	    addMatchingCerts(sel, certStores, forwardCerts, searchAllCertStores);
	}
	...
	if (!currentState.isInitial()) {
 	    // check for AuthorityInformationAccess extension
            ...
            if (aiaExt != null) {
		forwardCerts.addAll(getCerts(aiaExt));
	    }
	}
	
	if (debug != null) {
	    debug.println("ForwardBuilder.getMatchingCACerts: found " + 
		forwardCerts.size() + " forward certs");
	} 
	return forwardCerts;
    }

    private Collection<X509Certificate> 
	getCerts(AuthorityInfoAccessExtension aiaExt) {
        if (debug != null) {
	...
	ArrayList<X509Certificate> certs = new ArrayList<X509Certificate>();
		...
		LDAPCertStore.LDAPCertSelector ls = 
		    (LDAPCertStore.LDAPCertSelector) caSelector.clone();
                ls.setCertSubject(ls.getSubject());
                ls.setSubject(path);
		c = lcs.getCertificates(ls);
		if (debug != null) {
		    debug.println("ForwardBuilder.getCerts(AIA): found " + 
			c.size() + " certs");
		}
	    } catch (Exception e) {
		if (debug != null) {
		    debug.println("ForwardBuilder.getCerts(AIA): exception "
			+ "while fetching certs from CertStore: " + e);
		    e.printStackTrace();
		}
		continue;
	    }
	    certs.addAll(c);
	    if (!searchAllCertStores && !certs.isEmpty()) {
	        return certs;
	    }
	}
	return certs;
