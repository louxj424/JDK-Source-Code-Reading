...
public class PolicyFile extends java.security.Policy {
    ...
    /**
     * Initializes a bootstrap (sandbox) Policy object
     */
    public PolicyFile(boolean bootstrap) {
	if (!bootstrap) {
	    init((URL)null);
	} else {
	    PolicyInfo newInfo = new PolicyInfo(1);
...
*/
	...
	Boolean imp = (Boolean)AccessController.doPrivileged
	...
	for (int i = 0; i < entryPs.size(); i++) {
	    PolicyParser.PrincipalEntry pppe =
			(PolicyParser.PrincipalEntry)entryPs.get(i);
