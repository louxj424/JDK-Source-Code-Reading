...
public class HttpClient extends NetworkClient {
	...
	try {
		...
		public Object run() throws IOException {
			...
			s = server.getAddress().toString();
			int pos = s.indexOf('/');
			if (pos == 0) {
			    // extract the IP literal
			    s = s.substring(1);
			} else {
	...
	if (!usingProxy) {
	    ...
	    InetSocketAddress is = (InetSocketAddress)proxy.address();
	    String s = null;
	    if (is.isUnresolved()) {
		s = is.getHostName();
	    } else {
		// Use getAddress().toString() to avoid reverse lookup
		s = is.getAddress().toString();
		int pos = s.indexOf('/');
		if (pos == 0) {
