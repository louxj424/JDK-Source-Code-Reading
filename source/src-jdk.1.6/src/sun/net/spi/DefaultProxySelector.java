...
public class DefaultProxySelector extends ProxySelector {
    ...
    // By default we never use a proxy when connecting to localhost or one of
    // its aliases, this is for backward compatibility (see CR# 4924226)
    // However, in some special cases, like application testing, it is
    // needed to allow this. See CR# 6737819
    // This is achieved by using a special value for http.nonProxyHosts
    private static boolean okToProxyLocalhost = false;
	...
	if (nprop != null) {
	    String tmps = AccessController.doPrivileged(
		new PrivilegedAction<String>() {
		    public String run() {
			return NetProperties.get(nprop.property);
		    }
		});
	    if ("~localhost".equalsIgnoreCase(tmps)) {
		// Special case to disable the automatic exclusion
		// of Localhost when using a proxy
		okToProxyLocalhost = true;
	    }
	}

	// special case localhost and loopback addresses to
	// not go through proxy
	if (!okToProxyLocalhost && isLoopback(host)) {
